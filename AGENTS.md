...
> 本文件是 Codex 进入本项目时的第一上下文，承接 `~/.Codex/AGENTS.md` 全局规范。任何贡献者（含 AI agent）开工前必须先读完本文件，再动代码。
>
> Stack 调整提示：本项目前端默认 React + Vite + TS，后端 Tauri 2.x，agent 驱动层用 Codex Agent SDK。若需替换需先在本文件 §3 更新决策记录，再改实现，不允许反向。
...
- **名称**：cc-terminal
- **定位**：桌面版 Codex 风格 agent 编排客户端。以 Codex Agent SDK 为驱动层，在 Tauri 桌面壳内 host agent loop——自己控制 system prompt、注入工具、统一调度多实例。类比关系 ≈ Codex desktop 之于 ChatGPT。
- **核心价值**：
...
**驱动方式**：每个 agent = 一个 Node.js sidecar 进程，内部跑 `@anthropic-ai/Codex-agent-sdk` 的 `query()`。Rust 后端 spawn sidecar → stdin/stdout NDJSON 双向桥接 → 前端消费事件流渲染对话。

**认证**：SDK 自动读 `~/.Codex/.credentials.json` 的 OAuth 凭据（Pro/Max 订阅）。**不需要 ANTHROPIC_API_KEY**——Rust spawn sidecar 时显式 `env_remove("ANTHROPIC_API_KEY")` 防止环境变量覆盖 OAuth。

**默认模型**：`Codex-opus-4-6`（硬编码），v1.0 再做模型切换 UI。
...
- 用户手动点 **+ Thread** 派发新 agent（不自动 fan-out）
- 每个并行 agent 自动创建独立 git worktree（`<repo>/.cc-terminal-worktrees/<branch>`）
- 左栏多 Thread 并行，状态点实时更新
- 合并 worktree 时弹 Diff 面板，分块暂存 + inline comment 回灌 agent
...
| 设置 | 默认模型、主题切换；读写 `~/.Codex/settings.json` 的合法字段 |
| Skills / MCP | 装载与管理 |
...
| Agent 驱动 | `@anthropic-ai/Codex-agent-sdk`（Node.js sidecar） | 拿到 Codex 内核能力（skills/MCP/subagent/hooks），不走 CLI 隔靴搔痒 |
...
│       ├── claude_paths.rs    # 解析 ~/.Codex 目录的统一入口（只读）
...
**前端 → sidecar (stdin)：**
```
{"type":"hello","cwd":"<abs path>","systemPrompt":"...","model":"Codex-opus-4-6","allowedTools":[...],"permissionMode":"acceptEdits"}
...
- **路径**：`~/.Codex/projects/<project-hash>/<session-uuid>.jsonl`
- **格式**：每行一个 JSON
...
## 7. 红线（继承全局 AGENTS.md，加项目特化）

**即使在 auto-accept 模式下，碰到以下操作必须停下来问用户**：

1. 写入或修改 `~/.Codex/settings.json`、`~/.Codex/AGENTS.md`、`~/.Codex/.credentials.json`（用户私域）。
2. 触碰 `~/.Codex/projects/` 下任何 jsonl 文件——**只读**，损坏即丢失会话历史。
...
| 2026-04-29 | Step 1 脚手架完成 | Tauri 2 + React 19 + TS strict + Tailwind 3.4 + IBM Plex 三件套；三栏壳 + ThemeToggle + TokenGauge stub；AGENTS.md 受 robocopy `/XF` 保护未被覆盖 |
...
| 2026-04-29 | Step B 接入真实 jsonl | Rust `commands/tokens.rs` 流式扫 ~/.Codex/projects/**/*.jsonl，pricing.rs 单价表（v2026-04，含 Opus 1M 溢价）；前端删 fixtures.ts，TokensView + AppShell 通过 useTokenStats 共用真数据；3 个 pricing 单测全绿，clippy `-D warnings` 通过 |
| 2026-04-29 | Step C pty 桥接 Codex CLI | Rust `commands/chat.rs` 用 portable-pty 起子进程 + std::thread 阻塞读 reader 流式 emit；4 个命令 spawn_chat / chat_input / chat_resize / chat_kill；前端 PtyTerminal（xterm + FitAddon + ResizeObserver + 主题 MutationObserver）；ChatView 常驻 DOM 解决视图切换 kill session 问题；clippy 0 警告 |
...
| 2026-04-29 | Step D Skills 面板（只读版） | Rust `commands/skills.rs` 扫 ~/.Codex/skills/ + 子目录递归识别 plugin-namespaced；前端 SkillsView 双栏（左列表按命名空间分组 + 搜索过滤，右详情）；3 个 frontmatter 单测全绿。启停 / `/skill-vetter` 触发留 Step D2（涉 settings.json 红线） |
| 2026-04-29 | Step E 会话管理 | Rust `commands/sessions.rs` peek 每个 jsonl 头 64 行抓 cwd / firstTs，文件 mtime 当 lastTs；前端 SessionsView 按 cwd 分组 + 搜索；ChatView 通过 key={resumeId} 强制重建 + spawn `Codex --resume <uuid>` |
| 2026-04-29 | Step F Plugins 面板（只读版） | Rust `commands/plugins.rs` 读 installed_plugins.json + 每个 .Codex-plugin/plugin.json；前端 PluginsView 双栏 + 详情显示版本/作者/许可/路径；启停 / 升级 / 卸载留 Step F2 |
| 2026-04-29 | Step G Settings 面板（只读版） | Rust `commands/settings.rs` 读 settings.json + Codex --version；前端 SettingsView 展示 theme / effortLevel / 启用插件数 / 完整 raw JSON 折叠；明确写操作触发红线，留 Step G2 |
...
| 2026-04-30 | **v2 Pivot：CLI shell → Agent 编排客户端** | 从「包 CLI」升级到「embed Agent SDK」。删除 chat.rs / chat_json.rs / sessions / skills / plugins / settings / portable-pty。驱动层改 Codex Agent SDK + Node.js sidecar（pkg 打包单 .exe → Tauri externalBin）。UI 以 Codex 桌面为模板——三栏布局（ProjectList / ActiveThread / TaskSidebar），放弃 VSCode split pane 与 AutoClaw Kanban |
| 2026-04-30 | 认证策略：OAuth，不用 API key | Pro/Max 用户没有 API key。SDK 自动读 `~/.Codex/.credentials.json` OAuth 凭据。sidecar spawn 时 `env_remove("ANTHROPIC_API_KEY")` 防覆盖 |
| 2026-04-30 | 默认模型 = `Codex-opus-4-6` | 用户选定；v1.0 再做模型切换 UI |
